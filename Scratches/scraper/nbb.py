...
def get_verslag_from_nbb(id, jaar=2021):
    if id is None:
    ...
    data = response.json()
    return next(iter([{'id': d['id'], 'jaar': d['periodEndDateYear'], 'url': url_pdf.format(d['id'])} for d in data['content'] if d['periodEndDateYear'] == jaar]), None)


def download_verslag(url):
    if url is None:
